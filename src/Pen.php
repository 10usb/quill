	...
	private function element($parent, $element){
		...
		switch($this->path->getValue('display')){
			case 'block':
				...
				if($value = $this->path->getValue('border-top-width')) $style['border-top-width'] = $value->getMeasurement('pt');
				if($value = $this->path->getValue('border-top-style')) $style['border-top-style'] = $value;
				if($value = $this->path->getValue('border-top-color')) $style['border-top-color'] = $value;
				
				if($value = $this->path->getValue('border-right-width')) $style['border-right-width'] = $value->getMeasurement('pt');
				if($value = $this->path->getValue('border-right-style')) $style['border-right-style'] = $value;
				if($value = $this->path->getValue('border-right-color')) $style['border-right-color'] = $value;
				
				if($value = $this->path->getValue('border-bottom-width')) $style['border-bottom-width'] = $value->getMeasurement('pt');
				if($value = $this->path->getValue('border-bottom-style')) $style['border-bottom-style'] = $value;
				if($value = $this->path->getValue('border-bottom-color')) $style['border-bottom-color'] = $value;
				
				if($value = $this->path->getValue('border-left-width')) $style['border-left-width'] = $value->getMeasurement('pt');
				if($value = $this->path->getValue('border-left-style')) $style['border-left-style'] = $value;
				if($value = $this->path->getValue('border-left-color')) $style['border-left-color'] = $value;
